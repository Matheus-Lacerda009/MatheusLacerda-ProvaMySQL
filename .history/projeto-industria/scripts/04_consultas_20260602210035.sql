...
--22
select Produtos.nome, Categoria_Produto.tipo_categoria from Produtos
join Categoria_Produto
on Produtos.id_categoria = Categoria_Produto.id_categoria
group by Produtos.id_produto;

--23
select Produtos.nome, Fornecedores.cnpj, Produtos.preco_fabricacao from Produtos
join Fornecedores
on Produtos.id_fornecedor = Fornecedores.id_fornecedor
group by Produtos.id_produto;

--24
select nome from Produtos where preco_fabricacao = (select max(preco_fabricacao) from Produtos);

--25
select nome from Funcionarios where salario = (select avg(salario) from )